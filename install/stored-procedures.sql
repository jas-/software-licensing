...
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_def_add(IN `title` VARCHAR(128), IN `private` INT(1), IN `email` VARCHAR(64), IN `timeout` INT(10), IN `pkey` LONGTEXT, IN `pvkey` LONGTEXT, IN `pass` LONGTEXT, IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
