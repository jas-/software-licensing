...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE Configuration_def_get(IN `challenge` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE Configuration_def_keys(IN `challenge` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE Configuration_def_get_dn()
 DETERMINISTIC
 SQL SECURITY INVOKER
