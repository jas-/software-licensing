...
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_applications_add(IN `application` VARCHAR(255), IN `url` VARCHAR(255), `skey` LONGTEXT)
 DETERMINISTIC
...
BEGIN
 ...
 THEN
  UPDATE `configuration_applications` SET `url`=HEX(AES_ENCRYPT(url, SHA1(skey))) WHERE `resource`=SHA1(application) LIMIT 1;
 ELSE
  INSERT INTO `configuration_applications` (`resource`, `application`, `url`) VALUES (SHA1(application), HEX(AES_ENCRYPT(application, SHA1(skey))), HEX(AES_ENCRYPT(url, SHA1(skey)))) ON DUPLICATE KEY UPDATE `url`=HEX(AES_ENCRYPT(url, SHA1(skey)));
 END IF;
 SELECT ROW_COUNT() AS affected;
END//

DROP PROCEDURE IF EXISTS Configuration_applications_get//
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_applications_get(IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_applications_search(IN `dmn` VARCHAR(128), IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
