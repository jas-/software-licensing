...
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_access_del(IN `name` VARCHAR(32))
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='licensing'@'localhost' PROCEDURE Configuration_access_list(IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
