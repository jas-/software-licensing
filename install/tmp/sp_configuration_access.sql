...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_access_add(IN `name` VARCHAR(32), IN `filter` VARCHAR(255), `skey` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_access_del(IN `name` VARCHAR(32))
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_access_list(IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_access_get(IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
 COMMENT 'Returns list of access controls'
BEGIN
