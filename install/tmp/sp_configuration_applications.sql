...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_applications_add(IN `application` VARCHAR(255), IN `url` VARCHAR(255), IN `ip` LONGTEXT, `skey` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_applications_get(IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFH'@'localhost' PROCEDURE Configuration_applications_search(IN `dmn` VARCHAR(128), IN `skey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
