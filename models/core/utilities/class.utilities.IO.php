...
class io
{
 ...
 function __main($file)
 {
  ...
  if (!$this->__lockr($h)) {
   echo 'Could not set lock on opened file';
   return false;
  }

  if (!$this->__validate($this->__stat($file), $file)) {
   echo 'Problem found with file attributes after lock set';
   return false;
  }

  if (!$this->__magic($h, $file)) {
   echo 'File does not match list of allowed file types';
   return false;
  }
  echo 'All conditionals were met';
 }
